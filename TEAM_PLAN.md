...
| **Nitish** | **r3/graph-features DONE**: graph.py (8 cols), wired into featurize, 6 tests PASS; transfer_check.py (Task B) written; stub run: 63 feats, AUC 0.9993. Waiting for real candidates_train to measure OOF delta (threshold ≥ +0.002 to keep graph features on). EXPERIMENTS.md + methodology_notes.md updated. | real `candidates_train` from Dhanishkaa; real `folds_train` for transfer check | `scores_train` (OOF with graph features) → Faiz for decide tuning |
